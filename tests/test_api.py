    ...
    def test_test_auth(self):
        ...
        with aioresponses() as mocked:
            with pytest.raises(AuthError) as e_info:
                ...
                mocked.post(f"http://{TEST_HOST}/authentication/login",
                    status=500
                )
                loop.run_until_complete(HomePilotApi.test_auth(TEST_HOST,TEST_PASSWORD))
        ...
        with aioresponses() as mocked:
            ...
            mocked.post(f"http://{TEST_HOST}/authentication/login",
                status=200,
                headers={"Set-Cookie":"HPSESSION=V6EivFUCps1ItXmkymnsZLcpGJZL20keUtBAIvZxsbUaDGNP31sQ4YYxUT0XXv7P;Path=/"}
            )
